        ...
        <div class="container">
            ...
            <c:if test="${scheduleClicked}">
                <form action="saveSlot" method="post" class="p-3 rounded bg-light border shadow-sm">
                    <input type="hidden" name="specializationName" value="${selectedSpec}" />

                    <div class="mb-3">
                        <label for="doctorName" class="form-label fw-semibold">Doctor Name:</label>
                        <select class="form-select shadow-sm" id="doctorName" name="doctorName" required onchange="updateTimeSlots()">
...
<script>
    // Store slot data from backend
       const doctorSlots = {
           <c:forEach var="entry" items="${doctorAvailableSlots}" varStatus="status">
               "${entry.key}": [
                   <c:forEach var="slot" items="${entry.value}" varStatus="slotStatus">
                       "${slot}"<c:if test="${!slotStatus.last}">,</c:if>
                   </c:forEach>
       ...
       function updateTimeSlots() {
           ...
           if (doctorName && doctorSlots[doctorName]) {
               const slots = doctorSlots[doctorName];
